...
export default function FacilityAdminDashboard() {
  ...
  const [tab, setTab] = useState<
    "departments" | "users" | "service" | "reports"
  >("departments");
  ...
  const [serviceDelayInputs, setServiceDelayInputs] = useState<
  ...
  const [resetPasswordUser, setResetPasswordUser] = useState<ApiUser | null>(
    null,
  );
  ...
  const [newUserErrors, setNewUserErrors] = useState<
    Partial<
      Record<
        "fullName" | "username" | "role" | "departmentId" | "initialPassword",
        string
      >
    >
  >({});
  ...
  const [departmentSortKey, setDepartmentSortKey] =
    useState<DepartmentSortKey>("name");
  ...
  const loadFacilityAdminData = async () => {
    ...
    try {
      ...
      if (user?.facilityId) {
        try {
          const facilityServices = await trmsApi.getFacilityServices(
            user.facilityId,
          );
          nextFacilities = facilitiesData.map((facilityItem) =>
            facilityItem.id === user.facilityId
              ...
              : {
                  ...facilityItem,
                  services: Array.isArray(facilityItem.services)
                    ? facilityItem.services
                    : [],
                },
          );
        } catch {
  ...
  const facility =
    facilities.find((f) => f.id === user?.facilityId) || facilities[0] || null;
  ...
  const loadFacilityReportSummary = async (
    period: string,
    facilityId?: string,
  ) => {
    if (!facilityId) {
    ...
    try {
      ...
      const result: any = await trmsApi.getReport(
        buildFacilityReportQuery(period, facilityId),
      );
      setBackendReportSummary({
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `facility-report-${new Date().toISOString().split("T")[0]}.csv`;
      a.click();
  ...
  const hasFacilityReportData = Object.values(backendReportSummary).some(
    (value) => Number(value) > 0,
  );
  const handleCreateDepartment = async () => {
    const facilityId =
      newDepartment.facilityId || user?.facilityId || facility?.id || "";
  ...
  const handleToggleDepartmentActive = async (department: Department) => {
    ...
    try {
      ...
      setDepartmentActionError("");
      await trmsApi.updateDepartment(department.id, {
  ...
  const handleUpdateServiceStatus = async (
    ...
    try {
      ...
      const updated = await trmsApi.updateFacilityService(facilityId, {
        ...
        status,
        estimatedDelayDays:
          status === "limited" ? estimatedDelayDays : undefined,
      });
      setServiceDelayInputs((current) => ({
        ...current,
        [updated.id]:
          updated.estimatedDelayDays !== undefined &&
          updated.estimatedDelayDays !== null
  ...
  const handleCreateService = async () => {
    ...
    if (newService.status === "limited") {
      ...
      if (!raw) {
        setServiceActionError(
          "Estimated delay days is required for limited services.",
        );
        return;
      }
      ...
      if (!Number.isInteger(parsed) || parsed < 0) {
        setServiceActionError(
          "Estimated delay days must be a whole number (0 or greater).",
        );
        return;
      }
    ...
    try {
      ...
      setServiceDelayInputs((current) => ({
        ...current,
        [created.id]:
          created.estimatedDelayDays !== undefined &&
          created.estimatedDelayDays !== null
  ...
  const handleSaveServiceDelay = async (serviceId: string) => {
    const currentService = facility?.services?.find(
    ...
    if (raw === "") {
      setServiceActionError(
        "Please enter estimated delay days for limited services.",
      );
      return;
    }
  ...
  const facilityChartData = [
    ...
    {
      label: "Dept Heads",
      value: facilityManagedUsers.length,
      colorClass: "bg-emerald-500",
    },
    {
      label: "Services Up",
      value:
        facility?.services?.filter(
          (service) => normalizeServiceStatus(service.status) === "available",
        ).length || 0,
    ...
    },
    {
      ...
      value:
        facility?.services?.filter(
          (service) => normalizeServiceStatus(service.status) === "unavailable",
        ).length || 0,
  ...
  const handleCreateUser = async () => {
    ...
    if (!newUser.initialPassword.trim())
  ...
  useEffect(() => {
    // Derive role from selected department: liaison departments must have liaison_officer.
    const selected = departments.find((d) => d.id === newUser.departmentId);
    if (selected && selected.type === "liaison") {
      setNewUser((curr) => ({ ...curr, role: "liaison_officer" }));
      setRoleReadOnly(true);
    } else {
      // allow choosing department head when not liaison
      setRoleReadOnly(false);
    }
  }, [newUser.departmentId, departments]);

  const toggleDepartmentSort = (key: DepartmentSortKey) => {
    if (departmentSortKey === key) {
      setDepartmentSortDirection((current) =>
  ...
  const departmentTotalPages = Math.max(
    1,
    Math.ceil(sortedDepartments.length / PAGE_SIZE),
  );
  ...
  useEffect(() => {
    ...
    void loadFacilityReportSummary(
      reportPeriod,
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          <button
            ...
            className={tabCls(tab === "departments")}
          >
      ...
      <DashboardMiniChart
        title="Facility Operations Snapshot"
        ...
        <div className={card}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-bold">
              {t("fa.departments")} ({filteredDepartments.length})
            </h3>
            <button
              onClick={() => {
                resetAddDepartmentForm();
            ...
            <>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr
                      className={`border-b ${isDark ? "border-surface-700" : "border-surface-200"}`}
                    >
                      <th className="text-left pb-2 font-semibold text-surface-400 uppercase tracking-wide">
                        <button
                          onClick={() => toggleDepartmentSort("name")}
                          className="hover:text-primary-500 transition-colors"
                        >
                          {t("common.name")}
                          {departmentSortKey === "name"
                            ? departmentSortDirection === "asc"
                              ? " ↑"
                              : " ↓"
                            : ""}
                        </button>
                      ...
                      <th className="text-left pb-2 font-semibold text-surface-400 uppercase tracking-wide">
                        <button
                          onClick={() => toggleDepartmentSort("status")}
                          className="hover:text-primary-500 transition-colors"
                        >
                          {t("common.status")}
                          {departmentSortKey === "status"
                            ? departmentSortDirection === "asc"
                              ? " ↑"
                              : " ↓"
                            : ""}
                      ...
                      <th className="text-left pb-2 font-semibold text-surface-400 uppercase tracking-wide">
                        <button
                          onClick={() => toggleDepartmentSort("type")}
                          className="hover:text-primary-500 transition-colors"
                        >
                          Type
                          {departmentSortKey === "type"
                            ? departmentSortDirection === "asc"
                              ? " ↑"
                              : " ↓"
                            : ""}
                  ...
                  <tbody>
                    {pagedDepartments.map((dept) => {
                      ...
                      return (
                        <tr
                          key={dept.id}
                          className={`border-b last:border-0 ${isDark ? "border-surface-800" : "border-surface-100"}`}
                        >
                          ...
                          <td className="py-3 text-surface-500">
                            {(dept.type || "clinical").toUpperCase()}
                          </td>
                          <td className="py-3">
                            <div className="flex gap-1">
                              <button
                                ...
                                className={`px-2.5 h-8 rounded-lg text-[11px] font-semibold transition-colors disabled:opacity-60 ${
                                  isActive
              ...
              <div className="mt-3 flex items-center justify-between text-xs">
                <p
                  className={`${isDark ? "text-surface-400" : "text-surface-500"}`}
                >
                  Showing {pagedDepartments.length} of{" "}
                  {sortedDepartments.length}
                </p>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() =>
                      setDepartmentPage((current) => Math.max(1, current - 1))
                    }
                  ...
                  <span
                    className={`${isDark ? "text-surface-300" : "text-surface-600"}`}
                  >
                    Page {departmentPage} of {departmentTotalPages}
                  </span>
                  <button
                    onClick={() =>
                      setDepartmentPage((current) =>
                        Math.min(departmentTotalPages, current + 1),
                      )
                    }
        ...
        <div className={card}>
          ...
          {loading ? (
            ...
            <p className="text-sm text-surface-500">
              No department head users found.
            </p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
      ...
      {tab === "service" && (
        <div className={card}>
          <div className="flex items-center justify-between mb-4 gap-3">
            <h3 className="text-sm font-bold">
              {t("dir.serviceStatus")} — {facilityName}
            </h3>
            <button
              onClick={() => {
                setServiceActionError("");
          ...
          {showAddService && (
            <div
              className={`mb-4 p-4 rounded-xl border ${isDark ? "border-surface-800 bg-surface-950" : "border-surface-200 bg-surface-50"}`}
            >
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                ...
                <FormField
                  ...
                  hint={
                    newService.status === "limited"
                      ? "Required for limited services"
                      : "Used only when status is limited"
                  }
                />
                <div className="flex items-end">
                  <button
                    ...
                    className="w-full h-10.5 px-3 rounded-lg text-xs font-semibold bg-emerald-600 text-white hover:bg-emerald-700 transition-colors disabled:opacity-60"
                  >
            ...
            <div className="space-y-2">
              {facility?.services?.map((dept) => {
                ...
                const delayValue =
                  ...
                  (dept.estimatedDelayDays !== undefined &&
                  dept.estimatedDelayDays !== null
                    ? String(dept.estimatedDelayDays)
                    : "");
                return (
                  <div
                    key={dept.id}
                    className={`flex items-center gap-3 p-3.5 rounded-xl border ${isDark ? "border-surface-800 bg-surface-950" : "border-surface-200 bg-surface-50"}`}
                  >
                    ...
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold border ${sc.cls}`}
                    >
                      {sc.icon} {safeStatus}
                    </span>
                    <select
      ...
      {tab === "reports" && (
        <div className={card}>
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <select
                ...
                onChange={(e) =>
                  setReportPriority(e.target.value as typeof reportPriority)
                }
                className={`px-2.5 py-1.5 rounded-lg text-xs border outline-none ${isDark ? "bg-surface-950 border-surface-800 text-surface-100 focus:border-primary-400" : "bg-surface-50 border-surface-200 focus:border-primary-500"}`}
              >
          ...
          {reportNotice && (
            <p
              className={`mb-3 text-xs font-medium ${reportNotice.type === "success" ? "text-emerald-500" : "text-red-500"}`}
            >
          ...
          {reportLoading ? (
            <p className="text-sm text-surface-500">
              Loading backend report summary...
            </p>
          ) : !facility?.id && !user?.facilityId ? (
            <div
              className={`rounded-xl border p-4 text-sm ${isDark ? "border-surface-800 bg-surface-950 text-surface-300" : "border-surface-200 bg-surface-50 text-surface-600"}`}
            >
              No facility is linked to this account, so report data cannot be
              generated.
            </div>
            ...
            <div
              className={`rounded-xl border p-4 text-sm ${isDark ? "border-surface-800 bg-surface-950 text-surface-300" : "border-surface-200 bg-surface-50 text-surface-600"}`}
            >
              No report data for <strong>{reportPeriod}</strong>
              {reportPriority ? ` with ${reportPriority} priority` : ""}. Try
              another month, priority, or refresh after new activity.
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              {Object.entries(backendReportSummary).map(([key, val]) => (
                <div
                  key={key}
                  className={`rounded-xl border p-3 text-center ${isDark ? "border-surface-800 bg-surface-950" : "border-surface-200 bg-surface-50"}`}
                >
                  <p className="text-[10px] uppercase font-semibold text-surface-400 tracking-wide">
                    {key}
                  </p>
      ...
      {editingDepartment && (
        <Modal
          ...
          icon={
            <div
              className={`w-7 h-7 rounded-lg flex items-center justify-center ${isDark ? "bg-primary-500/20" : "bg-primary-100"}`}
            >
              <IconEdit
                size={14}
      ...
      {confirmDeleteDepartment && (
        <Modal
          ...
          icon={
            <div
              className={`w-7 h-7 rounded-lg flex items-center justify-center ${isDark ? "bg-red-500/20" : "bg-red-100"}`}
            >
              <IconAlertTriangle
                size={14}
        ...
        >
          <div className="space-y-4">
            <p
              className={`text-sm ${isDark ? "text-surface-300" : "text-surface-700"}`}
            >
              This will deactivate department{" "}
              <strong>{confirmDeleteDepartment.name}</strong>.
            </p>
            {departmentActionError && (
              <p className="text-xs text-red-500 font-medium">
                {departmentActionError}
            ...
            <div className="flex gap-3">
              ...
              >
                {deletingDepartmentId === confirmDeleteDepartment.id
                  ? "Deleting..."
                  : "Delete Department"}
              </button>
              <button
        ...
        >
          ...
          >
            <div className="mb-6 p-4 rounded-xl border border-primary-500/20 bg-primary-500/5">
              <label className="text-xs font-semibold text-primary-400 uppercase tracking-widest mb-3 block">
                Department Purpose
              </label>
              ...
              {departments.some((d) => d.type === "liaison") && (
                <p className="text-[10px] text-surface-500 mt-2 text-center">
                  A Liaison Unit already exists for this facility.
                </p>
              )}
            </div>
            ...
            <div className="flex gap-3 mt-4">
              ...
              >
                {creatingDepartment
                  ? "Creating..."
                  : `Create ${newDepartment.type === "liaison" ? "Liaison Unit" : "Department"}`}
              </button>
              <button
                type="button"
                onClick={() => {
        ...
        >
          ...
          >
            ...
            <FormField
              ...
              options={departments
                .filter((d) => {
                  if (newUser.role === "liaison_officer")
                    return d.type === "liaison";
                  if (newUser.role === "department_head")
                    return d.type === "clinical" || !d.type;
                  return true;
                })
