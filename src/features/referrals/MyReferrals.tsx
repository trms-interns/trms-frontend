...
export default function MyReferrals() {
  ...
  const myRefs = referrals.filter((r) => {
    ...
    // Direction filtering
    if (direction === "incoming" && r.receivingFacilityId !== user?.facilityId)
      return false;
    if (direction === "outgoing" && r.referringFacilityId !== user?.facilityId)
      return false;
  ...
  const getDisplayReferralId = (referral: {
    referralCode?: string;
    id: string;
  ...
  const getForwardErrorMessage = (error: unknown): string => {
    if (error instanceof TrmsApiError) {
      if (error.status === 403)
        return "You are not allowed to forward this referral.";
      if (error.status === 404)
        return "Referral or destination facility not found. Refresh and try again.";
      if (error.status === 400)
        return (
          error.message || "Please select facility and enter forwarding reason."
        );
  ...
  const selectedDS = selectedRef
    ? dischargeSummaries.find((ds) => ds.referralId === selectedRef.id)
    : null;
  ...
  const handleRejectReferral = async (): Promise<boolean> => {
    if (!selectedRef || !canTakeLifecycleAction || !rejectReason.trim())
      return false;
    try {
      setActionLoading(true);
      setActionError("");
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        ...
        <div className="lg:col-span-2 space-y-2">
          <div className={card}>
            <div
              className={`relative mb-3 rounded-lg border ${isDark ? "border-surface-700 bg-surface-950" : "border-surface-200 bg-surface-50"}`}
            >
              <IconSearch
                size={14}
                className={`absolute left-3 top-1/2 -translate-y-1/2 ${isDark ? "text-surface-500" : "text-surface-400"}`}
              />
              <input
                value={searchQuery}
              ...
              <div className="space-y-2">
                {myRefs.map((ref) => (
                  ...
                  >
                    ...
                    <p
                      className={`text-[10px] font-semibold uppercase tracking-wide ${isDark ? "text-surface-500" : "text-surface-400"}`}
                    >
                      Referral ID: {getDisplayReferralId(ref)}
                    </p>
                    <p
                      className={`text-xs truncate ${isDark ? "text-surface-400" : "text-surface-500"}`}
                    >
                      {ref.chiefComplaint}
                    </p>
        ...
        <div className="lg:col-span-3">
          {selectedRef ? (
            <div className={card}>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-base font-bold">
                    {selectedRef.patientName}
                  </h3>
                  <p
                    className={`text-xs ${isDark ? "text-surface-400" : "text-surface-500"}`}
                  >
                    {selectedRef.mrn} · {selectedRef.age}y, {selectedRef.sex}
                  </p>
                </div>
              ...
              <div
                className={`mb-5 flex items-center gap-2 rounded-xl border px-4 py-3 ${isDark ? "border-primary-500/20 bg-primary-500/10" : "border-primary-200 bg-primary-50"}`}
              >
                ...
                <div>
                  ...
                  <p className="text-base font-bold tracking-wide">
                    {getDisplayReferralId(selectedRef)}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  <p className="text-[10px] font-semibold text-surface-400 uppercase tracking-wide">
                    {t("ref.chiefComplaint")}
                  </p>
                  <p className="text-sm mt-0.5">{selectedRef.chiefComplaint}</p>
                </div>
                <div>
                  <p className="text-[10px] font-semibold text-surface-400 uppercase tracking-wide">
                    {t("ref.clinicalSummary")}
                  </p>
                  <p
                    className={`text-sm mt-0.5 leading-relaxed ${isDark ? "text-surface-300" : "text-surface-700"}`}
                  >
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div
                    className={`p-3 rounded-xl border ${isDark ? "border-surface-800 bg-surface-950/50" : "border-surface-100 bg-surface-50/50"}`}
                  >
                    <p className="text-[10px] font-semibold text-surface-500 uppercase tracking-wide">
                      Sent From
                    </p>
                    <p className="text-sm font-medium mt-1">
                      {selectedRef.referringFacility}
                    </p>
                  </div>
                  <div
                    className={`p-3 rounded-xl border ${isDark ? "border-surface-800 bg-surface-950/50" : "border-surface-100 bg-surface-50/50"}`}
                ...
                <div
                  className={`flex gap-4 text-xs ${isDark ? "text-surface-400" : "text-surface-600"}`}
                >
                  <span>
                ...
                {selectedRef.department && (
                  <div
                    className={`text-xs ${isDark ? "text-surface-400" : "text-surface-600"}`}
                  >
                ...
                <div
                  className={`rounded-xl border p-3 text-xs space-y-1.5 ${isDark ? "border-surface-700 bg-surface-950" : "border-surface-200 bg-surface-50"}`}
                >
                  <p className="font-semibold uppercase tracking-wide text-surface-500">
                    Referring User
                  </p>
                  <p>
                    ID:{" "}
                  ...
                  <p>
                    ...
                    >
                      {selectedRef.referringUserDepartment || "—"}
                    </strong>
                  </p>
                  <p>
                    Phone:{" "}
                    ...
                    >
                      {selectedRef.referringUserPhone || "—"}
                    </strong>
                  </p>
                  <p>
                    Email:{" "}
                    ...
                    >
                      {selectedRef.referringUserEmail || "—"}
                    </strong>
                  </p>
                </div>

                {(selectedRef.acceptedByUserId ||
                  selectedRef.acceptedByUserName) && (
                  <div
                    className={`rounded-xl border p-3 text-xs space-y-1.5 ${isDark ? "border-emerald-500/30 bg-emerald-500/5" : "border-emerald-200 bg-emerald-50"}`}
                  >
                    ...
                    <p>
                      ...
                      >
                        {selectedRef.acceptedByUserDepartment || "—"}
                      </strong>
                    </p>
                    <p>
                      Phone:{" "}
                      ...
                      >
                        {selectedRef.acceptedByUserPhone || "—"}
                      </strong>
                    </p>
                    <p>
                      Email:{" "}
                      ...
                      >
                        {selectedRef.acceptedByUserEmail || "—"}
                      </strong>
                    </p>
                  </div>
                )}

                {(selectedRef.rejectedByUserId ||
                  selectedRef.rejectedByUserName ||
                  selectedRef.rejectionReason) && (
                  <div
                    className={`rounded-xl border p-3 text-xs space-y-1.5 ${isDark ? "border-red-500/30 bg-red-500/5" : "border-red-200 bg-red-50"}`}
                  >
                    ...
                    <p>
                      ...
                      >
                        {selectedRef.rejectedByUserDepartment || "—"}
                      </strong>
                    </p>
                    <p>
                      Phone:{" "}
                      ...
                      >
                        {selectedRef.rejectedByUserPhone || "—"}
                      </strong>
                    </p>
                    <p>
                      Email:{" "}
                      ...
                      >
                        {selectedRef.rejectedByUserEmail || "—"}
                      </strong>
                    </p>
                    <p>
                      Reason:{" "}
                  ...
                  <div className="flex gap-2">
                    {canEditSentReferral && (
                      <button
                        onClick={() =>
                          navigate(
                            `/referrals/new?editReferralId=${selectedRef.id}`,
                          )
                        }
              ...
              {canConfirmOutbound && (
                <div className="mt-5 p-4 rounded-xl border space-y-3 border-primary-500/20 bg-primary-500/5">
                  ...
                  <p className="text-xs text-surface-400">
                    As a Liaison Officer, you must confirm this referral before
                    it is sent to the receiving facility.
                  </p>
                  <div className="flex gap-2">
                    <button
              ...
              {canTakeLifecycleAction && (
                <div
                  className={`mt-5 p-4 rounded-xl border space-y-3 ${isDark ? "border-surface-700 bg-surface-950" : "border-surface-200 bg-surface-50"}`}
                >
                      ...
                      <div>
                        ...
                        >
                          ...
                          {facilities
                            .filter(
                              (facility) =>
                                facility.id !== selectedRef.receivingFacilityId,
                            )
                            .map((facility) => (
              ...
              {selectedDS && (
                <div
                  className={`mt-5 p-4 rounded-xl border ${isDark ? "border-emerald-500/20 bg-emerald-500/5" : "border-emerald-200 bg-emerald-50"}`}
                >
                  <div className="flex items-center gap-2 mb-3">
                  ...
                  <div className="space-y-2 text-xs">
                    <p>
                      ...
                      {selectedDS.finalDiagnosis}
                    </p>
                    <p>
                      <strong>Treatment:</strong> {selectedDS.treatmentSummary}
                    </p>
                    <p>
                      <strong>Medications:</strong>{" "}
                      {selectedDS.medicationsPrescribed}
                    </p>
                    <p>
                    ...
                    <p className="text-surface-500">
                      Discharged: {selectedDS.dischargeDate} · By:{" "}
                      {selectedDS.createdByName}
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className={`${card} text-center py-16`}>
              <IconClipboardList
                size={32}
                className="mx-auto text-surface-500 mb-3"
              />
          ...
          <div className="space-y-4">
            ...
            {selectedRef && (
              <p
                className={`text-xs ${isDark ? "text-surface-400" : "text-surface-500"}`}
              >
                Patient: <strong>{selectedRef.patientName}</strong> · Referral
                ID: <strong>{getDisplayReferralId(selectedRef)}</strong>
              </p>
            )}
            ...
            {confirmAction === "forward" && (
              <>
                ...
                >
                  ...
                  <strong>{forwardFacilityId || "—"}</strong>
                </p>
                {forwardingNote.trim() && (
                  <p
                    className={`text-xs ${isDark ? "text-surface-400" : "text-surface-500"}`}
